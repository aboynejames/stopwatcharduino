...
const int touchpadbig = 3;  // big touchpad input number

const int LED = 8;  // pin for LED
const int BUTTON = 6; // start button input where pushbutton is connected
...
void setup(){
  ...
  pinMode(touchpadbig, INPUT);  // the bigtouchpad
...
void loop(){
  ...
  int touchpadbigReading = digitalRead(touchpadbig); 
  
  if(touchpadbigReading == HIGH )
  {

  Serial.println(touchpadbigReading);
    ...
    //digitalWrite(touchpadbigReading, LOW);
    touchpadbigReading = 0;
   //delay(10000);
